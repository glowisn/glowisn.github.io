export default function Description() {
  return (
    <>
      <h2 className="text-4xl font-bold mb-[0.5em]">협업과 배움을 통해 성장하는 개발자</h2>
      <p className="my-4 ">
        협업 마스터가 되고 싶은 프론트엔드 개발자 정예찬입니다.
        <br />
        혼자 보다 함께 할 때 더 큰 가치를 만들 수 있다고 믿습니다. 다양한 팀 프로젝트를 진행하며
        GitHub 등의 협업 툴은 물론, 팀원들과의 소통과 협업을 통해 더 나은 결과물을 만들어 왔습니다.
        <br />
        웹, 앱 기술들에 관심이 많아 Typescript 뿐만 아니라 Flutter, Firebase, Supabase, Vercel 등
        다양한 기술을 사용해 보았습니다.
        <br />
        어플리케이션을 더욱 좋게 만드는 것에 관심이 많아 <a href="https://github.com/swieomswieomlab/black_bean">
          쉬엄쉬엄
        </a>, <a href="https://github.com/boostcampwm2023/baekjoonrooms">백준룸즈</a> 등 다양한 프로젝트를 유지보수 하거나
        업그레이드 하고 있습니다.
        <br />
        <br />
        또한, 부스트캠프와 같은 교육 프로그램과 팀원들과 진행한 여러 프로젝트로 인해 스스로 배울 수 있고, 현재까지 많은 것들을 배워 비록 신입이지만 1~2년차 개발자 수준으로 성장했다고 생각합니다.
        <br />
        프론트엔드 뿐만 아니라 백엔드를 포함한 다양한 경험들을 쌓아 지속 가능한 개발자로 성장하고
        싶습니다.
        <br />
        <br />
        해당 페이지는 Next.js로 제작되었으며, GitHub Actions 및 GitHub Pages을 통해 배포되었습니다.{" "}
        <a href="https://github.com/glowisn/glowisn.github.io">깃허브</a>
      </p>
    </>
  );
}
